...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        concat_img = torch.cat((img, prev_img), dim=0)
...
class Test_Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        prev_img_file = ""
        # take the second frame if it's the start of an 
        if int(frame) == 0:
            prev_img_file = self.imgs[idx +1][0]
        else:
            prev_img_file = self.imgs[idx -1][0]
        # load image
        img = Image.open(img_file).convert('RGB')
        prev_img = Image.open(prev_img_file).convert('RGB')
        if self.transforms is not None:
            img, prev_img, _ = self.transforms(img, prev_img, None)
        
        concat_img = torch.cat((img, prev_img), dim=0)
        
        return concat_img, seq, frame
