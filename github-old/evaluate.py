...
def compute_AP(preds_list, labels_dict, iou_level=0.5):
    ...
    Total_positives = sum([len(x[1]) for x in labels_dict.items()]) # total number of fish
...
# Checks if the label is present in the predictions
def isRelevantLabel(label, preds):
    for p in preds:
        if(p[0] == label[0] and p[1] == label[1]):
            return True
    return False
...
def compute_mAP(preds, labels):
    ...
    for row in labels:
        if len(row) == 2 or not(isRelevantLabel(row,preds)):
            continue
...
if __name__=="__main__":
    mAP, AP = compute_mAP_from_files("/home/visum/predictions_slice_3d_14.csv", "/home/master/dataset/test/labels.csv")
